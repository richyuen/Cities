...
export function renderTitleScreen(ctx, nav) {
  ...
  const item = (label, onclick, opts = {}) => h('button', { class: `lc-menu-item${opts.primary ? ' primary' : ''}`, disabled: opts.disabled || undefined, onclick }, label);
  ...
  return h('div', { class: 'lc-menu-title-hero' },
    ...
    h('div', { class: 'lc-menu-list lc-menu-title-list' },
      item('Continue', () => nav.resume(), { primary: true }),
      item('New City', () => nav.push('newCity')),
      item('Load City', () => nav.push('saveLoad'), { disabled: !hasSaves }),
